...
#This file computes the data for Fig 3d.
...
# Main Execution
# ------------------------------------------------------------------------
# Computes the p_rb p_bb area plot for a given homophily value homsliceval
...
for i, prb in enumerate(prb_values):
    for j, pbb in enumerate(p_bb_values):
        Nr = 0.5 * N  # Example value for Nr
        M = ave_degree * N / ((1 - alpha) * (c2 * (c2 - 1)) + (alpha * c1 * (c1 - 1)))
        
        # Calculate critical probabilities for h1 = 0
        p_rr_crit_h1, hom1 = getslice([homsliceval], [0.1], p_rr, pbb, prb, N, Nr, M, alpha, c1, c2,x0)
        if not np.isnan(p_rr_crit_h1[0]):
            x0 = p_rr_crit_h1[0]
        h1val2 = (hom1[0]-(1-alpha)*homsliceval)/alpha
        p_rr_crit_h2, hom1 = getslice([h1val2], [homsliceval], p_rr, pbb, prb, N, Nr, M, alpha, c1, c2,x0)
        # Store the result
